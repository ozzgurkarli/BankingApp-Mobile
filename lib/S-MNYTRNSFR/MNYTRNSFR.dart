        ...
        child: Column(
          children: [
            ...
            ButtonSegment(
                value: '1',
                label: UText(
                  Localizer.Get(Localizer.account_no),
                  fontSize: 14,
                ))
          ],
            ...
            UButton(
                onPressed: () {
                  if(senderAccountValue == 0 || senderAccountValue == null){
                    setState(() {
                      senderAccountError = Localizer.Get(Localizer.this_field_cannot_be_left_empty);
                    });
                  }
                  if(recipientController.text.isEmpty){
                    setState(() {
                      recipientError = Localizer.Get(Localizer.this_field_cannot_be_left_empty);
                    });
                  }
                  if(nameSurnameController.text.isEmpty){
                    setState(() {
                      nameSurnameError = Localizer.Get(Localizer.this_field_cannot_be_left_empty);
                    });
                  }
                  if(amountController.text.isEmpty){
                    setState(() {
                      amountError = Localizer.Get(Localizer.this_field_cannot_be_left_empty);
                    });
                  }
                  ...
                  dtoTransfer.OrderDate = DateTime.parse(dateController.text.split('/')[2]+dateController.text.split('/')[1].padLeft(2,'0')+dateController.text.split('/')[0].padLeft(2,'0'));
                  
                  message.Add("DTOTransfer", dtoTransfer);
                  try{
                    UProxy.Post(
                      IService.START_MONEY_TRANSFER,
                      message);
                  }
                  catch(e){
                    HelperMethods.ApiException(context, e.toString(), popUntil: 1);
                  }

                  HelperMethods.SetSnackBar(context, Localizer.Get(Localizer.transaction_queued));
                  Navigator.pop(context);
